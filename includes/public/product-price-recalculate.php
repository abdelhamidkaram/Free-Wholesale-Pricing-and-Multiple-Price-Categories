...
function ddq_wholesale_quantity_or_zero($wholesale_number , $post_id ): float {
...
function ddq_wholesale_to_quantity_or_zero($wholesale_number , $post_id ): float {
...
function ddq_wholesale_price_or_base_price($wholesale_number , $post_id , $base_price): float {
...
function calculate_custom_price($quantity  ,$base_price ,  $post_id) {
   ...
   if ($quantity >= ddq_wholesale_quantity_or_zero(1 , $post_id) && $quantity < ddq_wholesale_to_quantity_or_zero(1 , $post_id)) {
      
      if(get_post_meta( $post_id, 'enable_wholesale1' , true) == 'no'){
         return $base_price ;
      }
      return ddq_wholesale_price_or_base_price(1 , $post_id, $base_price); 
   } 
   
   if ($quantity >= ddq_wholesale_quantity_or_zero(2 , $post_id)&& $quantity < ddq_wholesale_to_quantity_or_zero(2 , $post_id)) {
     
      if(get_post_meta( $post_id, 'enable_wholesale2' , true) == 'no'){
         return $base_price ;
      }
      return ddq_wholesale_price_or_base_price(2 , $post_id, $base_price); 
   }
   
   if ($quantity >= ddq_wholesale_quantity_or_zero(3 , $post_id)&& $quantity < ddq_wholesale_to_quantity_or_zero(3 , $post_id) ) {
     
      if(get_post_meta( $post_id, 'enable_wholesale3' , true) == 'no'){
         return $base_price ;
      }

      return ddq_wholesale_price_or_base_price(3 , $post_id, $base_price); 
   } 
