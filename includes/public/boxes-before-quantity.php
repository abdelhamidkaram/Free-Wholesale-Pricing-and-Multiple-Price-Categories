...
add_action('woocommerce_before_add_to_cart_quantity', 'fwpampc_add_message_before_quantity');

function fwpampc_add_message_before_quantity() {
    global $post;

    if (get_post_meta($post->ID, 'enable_wholesale', true) == 'yes') {
        echo '<div class="fwpampc-fields-box">';
        for ($i = 0; $i < 3; $i++) {
            
            if (get_post_meta($post->ID, 'enable_wholesale' . esc_attr($i + 1), true) == 'yes') {
                fwpampc_make_box($post, $i);
            }
        }
        echo '</div>';
    }
 ...
 function fwpampc_make_box($post, $i) : void {
    echo '<div class="fwpampc-field">';
    echo '<h5>';
